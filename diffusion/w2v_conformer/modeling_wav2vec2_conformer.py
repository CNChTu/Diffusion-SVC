...
class Wav2Vec2ConformerGroupNormConvLayer(nn.Module):
    def __init__(self, config, layer_id=0):
        super().__init__()
        self.in_conv_dim = config.conv_dim[layer_id - 1] if layer_id > 0 else config.hidden_size
        self.out_conv_dim = config.conv_dim[layer_id]
...
class AdaIN(nn.Module):
    def __init__(self, in_channel, style_dim, only_scale=False):
...
class Wav2Vec2ConformerEncoderLayer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.time_scale_and_shift_1 = AdaIN(embed_dim, embed_dim, only_scale=False)
        self.ffn1 = Wav2Vec2ConformerFeedForward(config)
        self.time_scale_1 = AdaIN(embed_dim, embed_dim, only_scale=True)

        # Self-Attention
        self.self_attn_layer_norm = nn.LayerNorm(embed_dim)
        self.self_attn_dropout = nn.Dropout(dropout)
        self.time_scale_and_shift_2 = AdaIN(embed_dim, embed_dim, only_scale=False)
        self.self_attn = Wav2Vec2ConformerSelfAttention(config)
        self.time_scale_2 = AdaIN(embed_dim, embed_dim, only_scale=True)
    ...
    def forward(
        ...
        hidden_states = self.time_scale_and_shift_1(hidden_states, timestep_emb)
        hidden_states = self.ffn1(hidden_states)
        hidden_states = self.time_scale_1(hidden_states, timestep_emb)
        hidden_states = hidden_states * 0.5 + residual
        residual = hidden_states

        # 2. Self-Attention layer
        hidden_states = self.self_attn_layer_norm(hidden_states)
        hidden_states = self.time_scale_and_shift_2(hidden_states, timestep_emb)
        ...
        hidden_states = self.time_scale_2(hidden_states, timestep_emb)
        ...
        hidden_states = self.time_scale_and_shift_3(hidden_states, timestep_emb)
...
class Wav2Vec2ConformerModel(Wav2Vec2ConformerPreTrainedModel):
    ...
    def forward(
        ...
        timestep_emb: Optional[torch.FloatTensor],
        attention_mask: Optional[torch.Tensor] = None,
        mask_time_indices: Optional[torch.FloatTensor] = None,
