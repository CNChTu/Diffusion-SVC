...
class RectifiedFlow(nn.Module):
    ...
    def sample_rk5(self, x, t, dt, cond):
        ...
        k_6 = self.velocity_fn(x + (-3 * k_1 + 2 * k_2 + 12 * k_3 - 12 * k_4 + 8 * k_5) * dt / 7, self.time_scale_factor * (t + dt),
                       cond)
    ...
    def sample_euler_fp64(self, x, t, dt, cond):
        x = x.double()
        x += self.velocity_fn(x.float(), self.time_scale_factor * t, cond).double() * dt.double()
        t += dt
        return x, t

    def sample_rk4_fp64(self, x, t, dt, cond):
        x = x.double()
        k_1 = self.velocity_fn(x.float(), self.time_scale_factor * t, cond).double()
        k_2 = self.velocity_fn((x + 0.5 * k_1 * dt.double()).float(), self.time_scale_factor * (t + 0.5 * dt), cond).double()
        k_3 = self.velocity_fn((x + 0.5 * k_2 * dt.double()).float(), self.time_scale_factor * (t + 0.5 * dt), cond).double()
        k_4 = self.velocity_fn((x + k_3 * dt.double()).float(), self.time_scale_factor * (t + dt), cond).double()
    ...
    def sample_rk5_fp64(self, x, t, dt, cond):
        x = x.double()
        k_1 = self.velocity_fn(x.float(), self.time_scale_factor * t, cond).double()
        k_2 = self.velocity_fn((x + 0.25 * k_1 * dt.double()).float(), self.time_scale_factor * (t + 0.25 * dt), cond).double()
        k_3 = self.velocity_fn((x + 0.125 * (k_2 + k_1) * dt.double()).float(), self.time_scale_factor * (t + 0.25 * dt), cond).double()
        k_4 = self.velocity_fn((x + 0.5 * (-k_2 + 2 * k_3) * dt.double()).float(), self.time_scale_factor * (t + 0.5 * dt),
                       cond).double()
        k_5 = self.velocity_fn((x + 0.0625 * (3 * k_1 + 9 * k_4) * dt.double()).float(), self.time_scale_factor * (t + 0.75 * dt),
                       cond).double()
    ...
    def OnnxExport(self, project_name, export_velocity, export_after, n_hidden, melbin):
        self.after = After(self.spec_min, self.spec_max)
        x = torch.randn((1, 1, melbin, 100), dtype=torch.float32)
        cond = torch.randn([1, n_hidden, 100], dtype=torch.float32)
        t = torch.tensor([0], dtype=torch.int64)
        if(export_velocity):
            torch.onnx.export(
                self.velocity_fn,
                (x.cpu(), t.cpu(), cond.cpu()),
                f"checkpoints/{project_name}/{project_name}_velocity.onnx",
